...
var monthArray = [];
var monthObj = {};
var mostPopularMonth = "";
var mostPopularMonthNumber = 0;
var salesPersonObj = {};
var topSalesPerson = "";
var topSalesPersonSales = 0;
var modelObj = {};
var topModel = "";
var topModelSales = 0;
var bankObj = {};
...
data.forEach(function a(e) {  //populates the list of months 
    totalProfit += e.gross_profit;
    monthArray.push(e.purchase_date.slice(5, 7))
})

monthArray.forEach(function b(e){ //populates the object with months and corresponding frequency
    let c = [];
    for(key in monthObj){
        c.push(key); //should push all the keys in the obj to array
    }
    if(c.includes(e) == false){
        monthObj[e] = 0;  //assigns new key to obj
    } else {
        monthObj[e] = monthObj[e] +1;//increases counter in obj
    }
})

for(key in monthObj){
    if(monthObj[key] > mostPopularMonthNumber){
        mostPopularMonthNumber = monthObj[key];
        mostPopularMonth = key;
    }
}

data.forEach(function f(e){
    let fullName = e.sales_agent.first_name + " " + e.sales_agent.last_name;
    if(salesPersonObj[fullName] != undefined){
        salesPersonObj[fullName] += e.gross_profit;
    } else {
        salesPersonObj[fullName] = e.gross_profit;
    }
    if(modelObj[e.vehicle.model] != undefined){
        modelObj[e.vehicle.model] = modelObj[e.vehicle.model] + 1;
    } else {
        modelObj[e.vehicle.model] = 1;
    }
    if(bankObj[e.credit.credit_provider] != undefined){
        bankObj[e.credit.credit_provider] = bankObj[e.credit.credit_provider] +1;
    } else {
        bankObj[e.credit.credit_provider] = 1;
    }
})

for(e in bankObj){
    if(bankObj[e] > topBankNum){
        topBankNum = bankObj[e];
        topBank = e;
    }
}

for (e in modelObj){
    if(modelObj[e] > topModelSales){
        topModelSales = modelObj[e];
        topModel = e;
    }
}

for(e in salesPersonObj){
    if(salesPersonObj[e] > topSalesPersonSales){
        topSalesPerson = e;
        topSalesPersonSales = salesPersonObj[e];
    }
}


console.log("Total profit: ",totalProfit);
console.log("The most popular month was ",mostPopularMonth,", with ", mostPopularMonthNumber, " sales.")
console.log("The top salesperson was ", topSalesPerson, ", with a total sales of ", topSalesPersonSales)
console.log("The top selling model was ", topModel, ", with a total sales of ", topModelSales)
console.log("The most common bank was ", topBank, ", with ", topBankNum, " loans.")